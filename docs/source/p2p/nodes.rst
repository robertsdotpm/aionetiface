...
Every technique for making P2P TCP connections in aionetiface ultimately passes
messages back to these handlers. Where aionetiface really shines is it's the only
...
in aionetiface does not depend solely on UPnP support.

.. literalinclude:: ../../examples/node_with_config.py
    :language: python3

aionetiface has a data directory in your home folder. This folder stores private
...
in aionetiface (this is useful when combined with socket options that allow servers to be instantly restarted.)
