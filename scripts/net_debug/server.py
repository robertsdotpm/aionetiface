    ...
    async def msg_cb(self, msg, client_tup, pipe):
        ...
        async def get_response():
            ...
            if p:
                ...
                if route is None:
                    return {"error": 3, "msg": "this server doesn't control that IP."}
            ...
            if p:
                ...
                if not valid_port(dest_port):
                    return {"error": 4, "msg": "invalid port for /reverse"}
                ...
                try:
                    ...
                    tcp_pipe = await Pipe(
                        ...
                        proto=TCP,
                        dest=dest_addr,
                        conf=dict_child({"con_timeout": 1}, NET_CONF),
                    ).connect()
                    ...
                    return {"error": 0, "msg": f"tcp service {dest_tup} is reachable."}
                except Exception:
                    return {
                        "error": 5,
            ...
            if p:
                ...
                return {
                    ...
                    "if": {"name": pipe.route.interface.name},
                }

            # Fallback and serve all other purposes.
            return {"error": 2, "msg": "method not implemented"}
...
async def start_net_debug():
    ...
    await net_debug_server.listen_all([i.rp[IP4], i.rp[IP6]], [20000], [TCP, UDP])
