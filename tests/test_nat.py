...
class TestNAT(AsyncTestCase):
    ...
    async def test_nat_intersect_range(self) -> None:
        tests = [
            [[2000, 10000], [5000, 20000], [5000, 10000]],
            [
                [2000, 10000],
                [2000, 10000],
                [2000, 10000],
        ...
        for test in tests:
            nat_a = nat_info(OPEN_INTERNET, delta_info(NA_DELTA, 0), test[0])

            nat_b = nat_info(OPEN_INTERNET, delta_info(NA_DELTA, 0), test[1])
    ...
    async def test_get_single_mapping(self) -> None:
        ...
        delta_val = 4
        params[-2] = nat_info(RESTRICT_NAT, delta_info(INDEPENDENT_DELTA, delta_val))
        ret = await get_single_mapping(*params)
        expected = [ret[0][0], last_mapped[1] + delta_val, 0]
        self.assertTrue(f_check(expected, ret))

        ##########################################################
        # Case 5 = Each new mapping is + delta value.
        # Only if local ports also increase sequentially.
        params[-2] = nat_info(RESTRICT_NAT, delta_info(DEPENDENT_DELTA, delta_val))
        expected = [last_mapped[0] + 1, last_mapped[1] + delta_val, 0]
        ret = await get_single_mapping(*params)
        self.assertTrue(f_check(expected, ret))

        ##########################################################
        # Case 6 = Reusable mapping from same local port.
        params[-2] = nat_info(FULL_CONE, delta_info(RANDOM_DELTA, 0))
